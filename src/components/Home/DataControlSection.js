			...
			mm.add("(pointer: fine)", () => {
				...
				const cleanups = cards.map((card) => {
					...
					const onMouseMove = (event) => {
						...
						if (icon && title) {
							gsap.to(icon, { x: percentX * 6, y: percentY * 5, duration: 0.35 });
							gsap.to(title, { x: percentX * -8, y: percentY * -5, duration: 0.35 });
						}
					};
					...
					const onMouseLeave = () => {
						...
						if (icon && title) {
							gsap.to([icon, title], { x: 0, y: 0, duration: 0.4, ease: "power2.out" });
						}
					};
			...
			<div className="mxxxl:container xl:container w-[90%] mx-auto text-center">
				...
				<h2 className="data-intro mt-4 text-[28px] md:text-[40px] font-semibold text-[#5C4AFF] [perspective:800px]">
					<span className="inline-flex flex-wrap justify-center gap-x-3 md:gap-x-4">
						{["Your", "Data.", "Your", "Control.", "Absolutely."].map((word) => (
							<span
								key={word}
