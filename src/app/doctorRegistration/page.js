  ...
  return (<main className="relative overflow-hidden bg-gradient-to-b from-[#eef4ff] via-[#f8eefe] to-white px-4 py-12 md:py-20"><HeroWaveBackground />
    ...
    <section className="relative mx-auto w-full max-w-6xl space-y-6 rounded-3xl border border-white/60 bg-white/80 p-4 shadow-[0_20px_60px_rgba(59,10,163,0.14)] backdrop-blur-sm sm:p-8 md:p-10">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className={`grid gap-4 overflow-hidden rounded-2xl border border-[#ece8fb] bg-white p-4 transition-all duration-700 ease-out sm:grid-cols-2 sm:p-6 ${showForm ? "max-h-[5000px] translate-y-0 opacity-100" : "pointer-events-none max-h-0 translate-y-6 opacity-0"}`}>
        ...
        <h2 className={sectionTitleClass}>5. Account & Access Settings</h2>
        <label className="sm:col-span-2 flex items-center gap-3 rounded-xl border border-[#ece8fb] p-3 text-sm text-[#2b2b43]"><input type="checkbox" className="size-4 accent-[#7b1fa2]" {...register("enableAdminAccess")} /> Enable Admin Portal Access</label>
        <input className={inputClass} placeholder="Username" {...register("username")} /><div /><input className={inputClass} type="password" placeholder="Password" {...register("password", { required: "Password is required." })} />
        <div><input className={inputClass} type="password" placeholder="Confirm Password" {...register("confirmPassword", { required: "Confirm password is required.", validate: (value) => value === watch("password") || "Passwords do not match." })} />{errors.confirmPassword && <p className="mt-1 text-xs text-red-500">{errors.confirmPassword.message}</p>}</div>
