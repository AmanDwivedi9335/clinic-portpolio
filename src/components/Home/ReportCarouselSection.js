  ...
  return (
    <section ref={sectionRef} className="bg-white py-10 md:py-14">
      <div className="report-intro mx-auto flex max-w-6xl flex-col items-center px-4 text-center md:px-8">
        <h2 className="text-2xl font-semibold tracking-tight text-[#6F1D96] md:text-4xl">
          Moment no one thinks about
        </h2>
      </div>
        ...
        >
          <CarouselContent className="py-4 md:py-6">
            {slides.map((slide, index) => (
              <CarouselItem key={`${slide.caption}-${index}`} className="basis-[84%] sm:basis-[47%] lg:basis-[33.33%]">
                <article
